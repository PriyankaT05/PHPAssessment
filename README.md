...
1. SERVER returns html generated by index.php to client OR
2. SERVER either sends a server information message or "Database refreshed!" message to the client. Javascript on the client loads message into the #mainContent element OR
3. SERVER either sends a server information message or HTTP response that presents the repository information retrieved from the MySQL database. Javascript on the client loads repository list items into the #mainContent element.
